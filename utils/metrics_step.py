...
class FID():
    ...
    def calculate_from_disk(self, generated_path, gt_path, step):
        """
        """
        if not os.path.exists(gt_path):
            raise RuntimeError('Invalid path: %s' % gt_path)
    ...
    def compute_statistics_of_path(self, path, verbose, discretize):
        ...
        if os.path.exists(npz_file) and False:
            ...
            d = dj[self.step] if discretize else 1

            path = pathlib.Path(path)
            files = list(path.glob('*.jpg')) + list(path.glob('*.png'))
            imgs = np.array([(cv2.resize(imread(str(fn)).astype(np.float32), (176, 256))) for fn in
                             files])  # np.array([imread(str(fn)).astype(np.float32) for fn in files])

            # Bring images to shape (B, 3, H, W)
            imgs = imgs.transpose((0, 3, 1, 2)) // d * d
    ...
    def get_activations(self, images, verbose=False):
        ...
        for i in trange(n_batches, desc='cal FID...'):
            if verbose:
...
class Reconstruction_Metrics():
    ...
    def calculate_from_disk(self, inputs, gts, save_path=None, sort=True, debug=0, step=1):
        """
        ...
        if os.path.exists(npz_file) and False:
            ...
            for index in trange(len(input_image_list), desc='cal PSNR, SSIM'):
                ...
                img_gt = (cv2.resize(img_gt, (176, 256)).astype(np.float32) // d * d) / 255.0
...
class LPIPS():
    ...
    def calculate_from_disk(self, path_1, path_2, batch_size=64, verbose=False, sort=True, step=1):
        ...
        d = dj[step]
        for i in trange(n_batches, desc='cal LPIPS'):
            if verbose:
                print('\rPropagating batch %d/%d' % (i + 1, n_batches))
                # end='', flush=True)
            start = i * batch_size
            ...
            imgs_1 = np.array(
                [cv2.resize(imread(str(fn)).astype(np.float32), (176, 256)) / 127.5 - 1 for fn in files_1[start:end]])
            imgs_2 = np.array(
                [cv2.resize(imread(str(fn)).astype(np.float32), (176, 256)) / 127.5 - 1 for fn in files_2[start:end]])

            imgs_1 = imgs_1.transpose((0, 3, 1, 2))
            imgs_2 = imgs_2.transpose((0, 3, 1, 2)) // d * d
...
dj = [0, 255 - 51, 255 - 51 * 2, 255 - 51 * 3, 255 - 51 * 4, 1]
...
if __name__ == "__main__":
    ...
    for step in range(5, 6):
        distorated_path = f'./results/step_dptn_steploss_{step}'

        gt_list, distorated_list = preprocess_path_for_deform_task(gt_path, distorated_path)

        FID = fid.calculate_from_disk(distorated_path, real_path, step=step)
        LPIPS = lpips_obj.calculate_from_disk(distorated_list, gt_list, sort=False, step=step)
        # print('LPIPS: ', LPIPS)
        REC = rec.calculate_from_disk(distorated_list, gt_list, distorated_path, sort=False, debug=False, step=step)
